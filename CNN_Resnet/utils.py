...
class EarlyStopper:
    ...
    """
    def __init__(self, verbose=False, patience=2, path='checkpoint.pt'):
        """Initialization.
    ...
    ####################
    #def val(model, valloader, loss_fn, device):
    #    model_before = MLP(model.img_width, model.channels, model.classes, 
    #                       num_hidden_units=model.hidden_units, 
    #                       num_hidden_layers=model.hidden_layers, act_fn=model.act)
    #    self.old, _, _ = validate(valloader, model_before, loss_fn, device, master_bar=None)
     #   self.new, _, _ = validate(valloader, model, loss_fn, device, master_bar=None)
     #   torch.save(model.state_dict(), self.path)
    def save_and_check(self, model, criterion):
        self.best = max(criterion[:-1])
        new = criterion[-1]
        
        diff = new - self.best
        if self.verb:
            print('Validation loss difference:', np.round(diff, 3))
        if diff < 0:
...
def run_training(model, optimizer, loss_function, device, num_epochs, 
    ...
    stopping = EarlyStopper(verbose, patience, path)
    ...
    for epoch in master_bar:
        ...
        if early_stopper and epoch > 2:
            ####################
            ## YOUR CODE HERE ##
            ####################
            stop = stopping.save_and_check(model, val_accs)
            if stop:
                break
            # END OF YOUR CODE #
